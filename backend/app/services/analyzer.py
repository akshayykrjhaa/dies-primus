...
def heuristic_file(info: FileInfo) -> dict[str, Any]:
    ...
    return {
        ...
        "summary": (
            ...
            "Add GROQ_API_KEY (or GEMINI_API_KEY) to backend/.env for a real explanation."
        ),
        "detail": (
            ...
            f"API key is configured. Add GROQ_API_KEY (primary) or GEMINI_API_KEY "
            f"(alternate) to backend/.env."
        ),
        "role": role,
...
def heuristic_project(
    ...
    return {
        ...
        "overview": (
            f"{slug} -- {description} "
            + (first_para or "")
            + " (Structural preview: add GROQ_API_KEY or GEMINI_API_KEY to backend/.env to have "
